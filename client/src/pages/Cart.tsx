...
import { buildWhatsAppUrl } from "@shared/catalogMessage";
...
export default function Cart() {
  ...
  const openWhatsApp = async () => {
    try {
      ...
      clearCart();
      window.location.assign(buildWhatsAppUrl(result.whatsappPhone, result.messageText));
    } catch (error) {
      toast.error("Não foi possível preparar a mensagem.", { description: error instanceof Error ? error.message : "Tente novamente em alguns instantes." });
    }
  };
    ...
    <div className="atelier-page bg-[#f4efe7] px-5 pb-24 pt-10 md:px-10 md:pt-16">
      <div className="mx-auto max-w-[1240px]">
        ...
        <div className="mt-9 grid gap-10 lg:grid-cols-[1.18fr_.82fr] lg:gap-16">
          <section>
            <div className="mb-7 flex items-end justify-between border-b border-[#241c18]/15 pb-6"><div><p className="eyebrow">Sua seleção</p><h1 className="display-font mt-3 text-5xl tracking-[-0.04em]">Escolhas em curso.</h1></div><span className="text-xs font-bold uppercase tracking-[0.13em] text-[#241c18]/55">{items.length} {items.length === 1 ? "item" : "itens"}</span></div>
          ...
          <aside className="self-start border border-[#241c18]/15 bg-[#ede4da] p-6 md:p-8"><p className="eyebrow">Resumo da escolha</p><div className="mt-7 space-y-4 text-sm"><div className="flex justify-between"><span className="text-[#241c18]/65">Subtotal</span><span>{formatBRL(subtotal)}</span></div><div className="border-t border-[#241c18]/15 pt-4"><div className="flex justify-between text-lg font-semibold"><span>Peças selecionadas</span><span>{items.reduce((total, item) => total + item.quantity, 0)}</span></div><p className="mt-2 text-xs leading-5 text-[#241c18]/55">A disponibilidade e a entrega serão confirmadas no atendimento pelo WhatsApp.</p></div></div><button onClick={openWhatsApp} disabled={requestMutation.isPending} className="btn-primary mt-8 flex w-full justify-center disabled:cursor-wait disabled:opacity-70">{requestMutation.isPending ? "Preparando mensagem..." : <><MessageCircle size={16} /> Solicitar no WhatsApp</>}</button><p className="mt-5 text-center text-[10px] font-bold uppercase tracking-[0.13em] text-[#241c18]/50">Sua seleção será enviada em uma mensagem pronta</p></aside>
        </div>
      </div>
    </div>
  );
